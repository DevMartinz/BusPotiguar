...
import { getEditableTableData, EditableTableRow, Pagination } from 'api/table.api';
...
export const EditableTable: React.FC = () => {
  ...
  const [tableData, setTableData] = useState<{ data: EditableTableRow[]; pagination: Pagination; loading: boolean }>({
    data: [],
  ...
  const isEditing = (record: EditableTableRow) => record.key === editingKey;

  const edit = (record: Partial<EditableTableRow> & { key: React.Key }) => {
  ...
  const save = async (key: React.Key) => {
    try {
      const row = (await form.validateFields()) as EditableTableRow;
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'actions',
      width: '15%',
      render: (text: string, record: EditableTableRow) => {
  ...
  const mergedColumns = columns.map((col) => {
    ...
    return {
      ...col,
      onCell: (record: EditableTableRow) => ({
        record,
